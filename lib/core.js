...
var Core = {
  ...
  getCommand(hostname, jump) {
    ...
    var shellSyntaxCommand = 'ssh -A -t ' + this.getHost(Config.username, hostname);

    if (jump) {
      shellSyntaxCommand = 'ssh -A -t ' + this.getHost(Config.username, this.getJumpHost()) + ' ' + shellSyntaxCommand;
    }
